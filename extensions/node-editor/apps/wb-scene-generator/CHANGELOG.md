...
- **Correction: `file:` in this app's `package.json` is not the install path.** The Unreleased bullet below that switched `@forgeax/node-runtime` to `file:../../packages/node-runtime` for Studio `bun install` is superseded. Apps keep `workspace:*`; Studio installs `@forgeax-extension/wb-scene-generator` from `release/`. SSOT: [`docs/development-and-release.md`](../../docs/development-and-release.md). *为什么：* relative `file:` dies outside this tree; mixing a pnpm workspace member with a Bun-installable extension already failed twice.
