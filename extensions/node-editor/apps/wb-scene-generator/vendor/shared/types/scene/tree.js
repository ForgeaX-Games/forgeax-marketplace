...
export function splitPath(path) {
    ...
    for (const s of segs) {
        if (!s)
            throw new Error(`SceneTree: empty segment in path "${path}"`);
    }
    return segs;
}
/** 段名拼接成路径。 */
...
function findChildIdx(children, name) {
    let lo = 0;
...
function insertChildSorted(children, child) {
    ...
    while (lo < hi) {
        const mid = (lo + hi) >>> 1;
        if (children[mid].name.localeCompare(child.name) < 0)
            lo = mid + 1;
        else
            hi = mid;
    }
...
export function readNode(root, path) {
...
export function listChildren(root, path) {
 ...
 */
function rewriteAtPath(node, segs, segIdx, newVersion, mutator) {
 ...
 */
export function upsertCells(root, path, data, newVersion) {
    const segs = splitPath(path);
    if (segs.length === 0) {
...
export function setTransform(root, path, transform, newVersion) {
    const segs = splitPath(path);
    if (segs.length === 0) {
    ...
    const frozenT = Object.freeze({ ...transform });
    return rewriteAtPath(root, segs, 0, newVersion, (current) => {
        return Object.freeze({ ...current, transform: frozenT, version: newVersion });
    });
}
/**
 ...
 */
export function setAttribute(root, path, key, value, newVersion) {
...
export function getAttribute(root, path, key) {
    if (typeof key !== 'string' || key.length === 0) {
 ...
 */
function recloneSubtree(src, newName, newPath, newVersion) {
    const children = Object.freeze(src.children.map(c => recloneSubtree(c, c.name, `${newPath}/${c.name}`, newVersion)));
    return Object.freeze({
        name: newName,
        path: newPath,
        version: newVersion,
 ...
 */
export function graftAt(root, destPath, source, newVersion) {
 ...
 */
export function upsertSubtree(root, destPath, source, newVersion) {
    const segs = splitPath(destPath);
    if (segs.length === 0) {
