/**
 * g_joint_floating —— 追加 `id = joint(type="floating", parent, child)`。
 *
 * 自由 6-DoF 关节；child 相对 parent 没有约束（用于浮动基座 / 调试可视化）。
 * URDF 渲染时不输出 axis / limit。
 */
...
export function gJointFloating(input: Record<string, unknown>): Record<string, unknown> {
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'jflo');
