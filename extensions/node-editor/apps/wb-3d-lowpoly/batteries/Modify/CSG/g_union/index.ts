...
export function gUnion(input: Record<string, unknown>): Record<string, unknown> {
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'union');
  if (!isValidId(id)) return { geometry: incoming, id: '', error: `invalid id "${id}"` };

  return { geometry: emit(incoming, id, 'union', { a: ref(aId), b: ref(bId) }), id };
}
