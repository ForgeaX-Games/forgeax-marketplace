 ...
 * 是否走宿主图像网关。
 *
 *   - 嵌入宿主（path 含 /extensions/wb-reel）→ 同源 /__ce-api__ 命中 forgeax-server → 默认开
...
export function shouldUseHostImageGateway(): boolean {
  ...
  // 嵌入宿主有两种形态：① host serveStatic 在 /extensions/wb-reel 子路径下；
  // ② 独立插件 dev —— 宿主把 wb-reel iframe 到它自己的端口（:15175，路径 '/'）。
  // 形态②路径里没有 /extensions/wb-reel，但宿主 vite 仍把 /__ce-api__ 反代到
  // forgeax-server → litellm，所以「被 iframe 嵌入」即可默认走宿主网关
  // （key 全留 server）。只有真正独立打开（非 iframe）才回落到直连/Mock。
  if (window.location.pathname.includes('/extensions/wb-reel')) return true
  try {
