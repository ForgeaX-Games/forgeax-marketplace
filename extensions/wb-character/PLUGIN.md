...
- **挂载点**: `packages/marketplace/extensions/wb-character/` (二级 submodule，宿主仓 = `ForgeaX-Games/forgeax-marketplace`)
- **Plugin manifest**: `forgeax-extension.json` — kind=workbench, 12 tools, 全部 exposedToAI

## 双模态契约

- **iframe 嵌入**: studio host 把 `dist/index.html` 当 static-serve 到 `/extensions/wb-character/`，Sidebar 一个 ⚒️ tab 起 iframe，中间是试 gameplay 视口
