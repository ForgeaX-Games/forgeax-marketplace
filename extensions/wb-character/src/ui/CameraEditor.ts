...
export class CameraEditor {
  ...
  private buildHTML(): string {
    return `
      ...
      <div class="cam-live-info">
        <div class="cam-row"><span class="cam-label">${t('cam.label.position')}</span><span data-live="pos">—</span></div>
        <div class="cam-row"><span class="cam-label">${t('cam.label.target')}</span><span data-live="tgt">—</span></div>
        <div class="cam-row"><span class="cam-label">${t('cam.label.fov')}</span><span data-live="fov">—</span></div>
      </div>

      <div class="cam-save-row">
        <input data-cam="preset-name" type="text" placeholder="${t('cam.presetName.placeholder')}" value="default" />
        <button data-action="save-preset">${t('cam.save')}</button>
      </div>

      <div class="cam-actions">
        <button data-action="copy">${t('cam.copy')}</button>
        <button data-action="reset">${t('cam.reset')}</button>
      </div>

      <div class="cam-presets-section">
        <div class="cam-presets-title">${t('cam.presetsTitle')}</div>
        <div data-presets></div>
      </div>

      <div class="cam-manual-toggle">
        <button data-action="toggle-manual">${t('cam.manual.toggle')}</button>
      </div>
  ...
  private bindEvents(): void {
    this.overlay.querySelector('[data-action="save-preset"]')?.addEventListener('click', () => {
      ...
      this.renderPresets()
      this.flashButton('[data-action="save-preset"]', t('cam.savedFlash'))
    })
    ...
    this.overlay.querySelector('[data-action="copy"]')?.addEventListener('click', () => {
      ...
      navigator.clipboard.writeText(json).then(() => {
        this.flashButton('[data-action="copy"]', t('cam.copiedFlash'))
      }).catch(() => {})
    })
    ...
    this.overlay.querySelector('[data-action="toggle-manual"]')?.addEventListener('click', () => {
      ...
      btn.textContent = hidden ? t('cam.manual.toggleOpen') : t('cam.manual.toggle')
  ...
  private renderPresets(): void {
    ...
    if (presets.length === 0) {
      container.innerHTML = `<div style="color:var(--text-secondary);font-size:11px;padding:4px 0;">${t('cam.presets.empty')}</div>`
      return
    }
