...
const PROFILE_OPS: Record<string, string> = {
  spur:        'planetary_gearset',
  herringbone: 'herringbone_planetary_gearset',
};

export function gPlanetaryGearset(input: Record<string, unknown>): Record<string, unknown> {
  const incoming = parseGeometryPort(input.geometry) ?? makeGeometry();

  const profile = String(input.tooth_profile ?? 'spur').trim().toLowerCase();
  const op = PROFILE_OPS[profile];
  if (!op) {
    return { geometry: incoming, id: '', error: `planetary_gearset: tooth_profile must be one of spur/herringbone, got "${profile}"` };
  }
  ...
  if (![mod, width, rim].every(Number.isFinite)
    ...
    return { geometry: incoming, id: '', error: 'planetary_gearset: module/width/rim_width must be finite, sun/planet teeth must be >= 3, n_planets >= 1' };
  }
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'plan');
  if (!isValidId(id)) {
...
export default gPlanetaryGearset;
