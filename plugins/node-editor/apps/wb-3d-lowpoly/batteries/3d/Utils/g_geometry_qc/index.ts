...
export function gGeometryQc(input: Record<string, unknown>): Record<string, unknown> {
  ...
  //    (extrude/revolve/loft/union/difference/...)、没有 Parts/Gears/Architecture
  ...
  if (primitiveOnly) {
    advisories.push({
      ...
      message:
        ...
        `(no CSG solid, no Parts/Gears/Architecture, no baked mesh) — ` +
        `even wrapped in g_part + g_joint this is still a "stacked boxes" decomposition. ` +
        `model each part for real first (g_difference/g_revolve/... or a semantic Parts/Gears op, ` +
