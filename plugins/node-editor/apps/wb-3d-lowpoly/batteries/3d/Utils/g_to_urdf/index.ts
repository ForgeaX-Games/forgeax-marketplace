...
function emitImplicitLinkForShape(
  ...
  out.push(`    </visual>`);
  // collision 同形（再调一次以获得独立的 provenance 记录）
  const collisionXml = renderShapeGeometry(shape, bakedUrls, byId, state, {
    linkId: linkName,
    kind: 'collision',
  });
  out.push(`    <collision>`);
...
function emitPart(
  ...
  if (!shape) {
    ...
    if (collisionStmts && collisionStmts.length > 0) {
      ...
      // visual = collision：再调一次同 shape 的渲染，但把 provenance 标记为 collision
      const collisionGeomXml = renderShapeGeometry(shape, bakedUrls, byId, state, {
        linkId: s.id,
        kind: 'collision',
      });
      out.push(`    <collision>`);
