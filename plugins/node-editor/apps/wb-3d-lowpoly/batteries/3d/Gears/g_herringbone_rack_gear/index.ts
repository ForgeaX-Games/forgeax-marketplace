...
export function gHerringboneRackGear(input: Record<string, unknown>): Record<string, unknown> {
  ...
  if (![mod, length, width, height].every(Number.isFinite)) {
    return { geometry: incoming, id: '', error: 'herringbone_rack_gear: numeric params must be finite' };
  }
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'hrack');
  if (!isValidId(id)) {
    return { geometry: incoming, id: '', error: `invalid id "${id}"` };
  }

  const next = emit(incoming, id, 'herringbone_rack_gear', args);
  return { geometry: next, id };
}

export default gHerringboneRackGear;
