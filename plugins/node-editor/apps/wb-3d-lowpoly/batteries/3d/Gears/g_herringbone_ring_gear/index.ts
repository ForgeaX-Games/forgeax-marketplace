...
export function gHerringboneRingGear(input: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!Number.isFinite(mod) || mod <= 0 || !Number.isFinite(width) || width <= 0 || !Number.isFinite(rim) || rim <= 0 || !Number.isInteger(teeth) || teeth < 3) {
    return { geometry: incoming, id: '', error: 'herringbone_ring_gear: module/width/rim_width must be positive and teeth_number must be an integer >= 3' };
  }
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'hring');
  if (!isValidId(id)) {
    return { geometry: incoming, id: '', error: `invalid id "${id}"` };
  }

  const next = emit(incoming, id, 'herringbone_ring_gear', args);
  return { geometry: next, id };
}

export default gHerringboneRingGear;
