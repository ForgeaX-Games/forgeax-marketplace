...
export function gSpurGear(input: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!Number.isFinite(mod) || mod <= 0 || !Number.isFinite(width) || width <= 0 || !Number.isInteger(teeth) || teeth < 3) {
    return { geometry: incoming, id: '', error: 'spur_gear: module/width must be positive and teeth_number must be an integer >= 3' };
  }
  ...
  const id = rawId !== '' ? rawId : freshId(incoming, 'spur');
  if (!isValidId(id)) {
    return { geometry: incoming, id: '', error: `invalid id "${id}"` };
  }

  const next = emit(incoming, id, 'spur_gear', args);
  return { geometry: next, id };
}

export default gSpurGear;
