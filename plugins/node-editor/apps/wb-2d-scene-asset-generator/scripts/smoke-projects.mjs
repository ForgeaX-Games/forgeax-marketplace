...
// FORGEAX_PROJECT_ROOT + an ALT port (never touches :9567).
//
...
const seedRt = createRuntime({ projectRoot: root, pipelineId: 'main', pluginId: '@forgeax-extension/wb-2d-scene-asset-generator' })
...
try {
  ...
  // ── PHASE 2: AI/headless path — create → open → batch ─────────────────────
  ...
  console.log('[smoke-projects] PHASE 2 OK — AI create→open→batch lands in the opened project + history')
...
if (process.exitCode) {
  ...
  console.log('[smoke-projects] OK — backfill + 2 isolated projects (graph+history) + activate switching in API + AI create/open/batch + safe delete')
}
