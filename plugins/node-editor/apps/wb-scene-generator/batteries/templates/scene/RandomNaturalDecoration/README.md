...
| portName | 类型 | 语义 | 典型去向 |
|---|---|---|---|
| `out_0` | scene | 装饰产物（主产物） | → `tree_merge.item_N`（汇总） |
| `out_1` | string | NaturalDecPath（路径句柄） | 一般不接 |
| `out_2` | scene | **NaturalDec** 装饰 | 一般不接（out_0 已是主产物） |
| `out_3` | scene | **Rest** 剩余空地 | 通常是最后一环，可不接 |
| `out_4` | string | RestPath（剩余路径句柄） | 一般不接 |

> 注意：本组 `out_1` 是 **string**（NaturalDecPath），不是场景；剩余 Rest 在 `out_3`。
...
{ "type":"connect","edgeId":"e_dec_out0","source":{"nodeId":"<G_DECO>","port":"out_0"},     "target":{"nodeId":"merge_all","port":"item_2"} }
```
...
forgeax node connect --edge-id e_dec_out0 --from deco:out_0         --to merge_all:item_2 $G
```
