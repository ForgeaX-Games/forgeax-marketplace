# EdgeGreenClusters（边缘绿簇）

> templateId（传给 `scene:pipeline.instantiateTemplate`）：`group_edge_green_clusters`，也可用 basename `EdgeGreenClusters`。
> 内部 13 个节点、1 个嵌套子组（TileAssetName）。实例化后返回全新运行时 `groupId`，后续连线一律用返回值。

## 功能说明

在上游区域（scene）的**边缘**点缀一圈形状不规则的绿簇（灌木/苔藓/藻类等碎绿），粘附在区域内缘。它**只叠加装饰、不消耗区域**：把绿簇作为一个 tile 图层挂回上游 scene，原区域原样保留在 `out_0`。

内部数据流：`scene → node_explode → rect_grid + voxel_slice`（取占用区 grid）`→ edge_green_cluster`（沿边缘生簇）`→ grid2node → add_child`（挂回上游 scene）。
...
| portName | 类型 | 语义 | 典型去向 |
|---|---|---|---|
| `out_0` | scene | 主产物（原区域 + 边缘绿簇） | → `tree_merge.item_N` 或下一组 `in_0` |
| `out_1` | scene | **Clusters** 绿簇本体图层 | 一般不接 |
| `out_2` | string | ClustersPath（路径句柄） | 一般不接 |
...
{ "toolId":"scene:pipeline.instantiateTemplate","caller":{"kind":"ai"},
  "args":{ "templateId":"EdgeGreenClusters", "position":{"x":-500,"y":2600},
           "opts":{"actor":"ai:sino","label":"实例化 EdgeGreenClusters"} } }
```
...
- 给已成形的**地块 / 水体 / 广场**边缘加自然碎绿（灌木、苔藓、藻类、滩涂植被）。
- 纯装饰叠加，不改变原区域；`out_0` 仍可继续往下游传递。
