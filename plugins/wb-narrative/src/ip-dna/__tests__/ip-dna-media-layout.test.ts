...
describe("媒体优先布局（§6.1）· 家族映射与路径", () => {
  ...
  it("各阶段路径形态符合 §6.1", () => {
    ...
    expect(norm(mediaOriginalDir(ts, title, "book"))).toContain(`book/story_book/story_book_original/${ts}_${title}`);
    expect(norm(processingDir(ts, title, "book"))).toContain(`book/story_book/story_book_processing/${ts}_${title}`);
    expect(norm(extractionOutputDir(ts, title, "book"))).toContain(`book/story_book/story_book_extraction_output/${ts}_${title}`);
    ...
    // 混合模态统一落主媒体 book。
    expect(norm(extractionOutputDir(ts, title, "mixed"))).toContain("book/story_book/story_book_extraction_output");
  });
});
...
describe("媒体优先布局 · 混合模态分家落盘", () => {
  it("文本/图片/视频分别落 book/picture/video 各 *_original，manifest 路径为媒体优先相对路径", () => {
    ...
    expect(paths).toContain(`book/story_book/story_book_original/${ts}_${title}/正文.txt`);
