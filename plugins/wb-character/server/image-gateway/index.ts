 ...
 *  forward it to the owning vendor so fallbacks use their own default model
 *  instead of 404-ing on a foreign model id. */
function modelOverrideForVendor(
  vendorName: string,
  req: ImageGenRequest,
  overrideOwner: string | undefined,
): string | undefined {
  if (!req.modelOverride) return undefined;
  if (vendorName === overrideOwner) return req.modelOverride;
  return undefined;
}
...
export function resolveChain(req: ImageGenRequest): string[] {
  ...
  let chain: string[];
  if (!req.vendor || !vendors.has(req.vendor)) chain = natural;
  else chain = [req.vendor, ...natural.filter((id) => id !== req.vendor)];

  // Seedream is text-to-image only — it never forwards refImageBase64 and would
  // redraw a brand-new character (the "画风对不上" bug). When a reference portrait
  // is supplied, route through reference-capable vendors. azure-gpt-image
  // (/images/edits) is the only one wired with working credentials; nano-banana
  // (Gemini) needs GEMINI_API_KEY; litellm-images is the proxy fallback.
  if (req.refImageBase64?.trim()) {
    const refFirst = ['azure-gpt-image', 'nano-banana', 'litellm-images'];
    if (req.vendor && req.vendor !== 'seedream') {
      chain = [req.vendor, ...refFirst.filter((id) => id !== req.vendor)];
    } else {
      chain = [...refFirst];
    }
    chain = chain.filter((name) => name !== 'seedream');
  }
  return chain;
}
  ...
  for (const name of chain) {
    ...
    try {
      const r = await v.generate({
        ...
        modelOverride: modelOverrideForVendor(name, req, overrideOwner),
      });
      return {
        ...r,
